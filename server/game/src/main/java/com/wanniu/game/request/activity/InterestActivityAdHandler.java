    ...
    public PomeloResponse request() throws Exception {
        return new PomeloResponse() {
            protected void write() throws IOException {
                WNPlayer player = (WNPlayer) InterestActivityAdHandler.this.pak.getPlayer();
                ActivityHandler.InterestActivityAdResponse.Builder res = ActivityHandler.InterestActivityAdResponse.newBuilder();

                long now = System.currentTimeMillis();
                Map<Integer, ActivityExt> props = GameData.Activitys;

                ArrayList<ActivityHandler.ActivityListInfo> list_info = new ArrayList<>();
                for (ActivityCO p : props.values()) {
                    ...
                    if (prop.beginTime < now && now < prop.endTime) {
                        ...
                        ActivityHandler.ActivityListInfo.Builder info = ActivityHandler.ActivityListInfo.newBuilder();
