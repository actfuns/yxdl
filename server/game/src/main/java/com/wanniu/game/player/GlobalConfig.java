...
public class GlobalConfig {
        ...
        for (ParametersCO param : GameData.Parameterss.values()) {
            try {
                ...
                if ("int".equals(typeName)) {
                    field.set(config, Integer.valueOf(Integer.parseInt(param.paramValue)));
                    continue;
                }
                if ("float".equals(typeName)) {
                    field.set(config, Float.valueOf(Float.parseFloat(param.paramValue)));
                    continue;
                }
                field.set(config, param.paramValue);
            } catch (Exception e) {
                Out.error(new Object[]{param.paramName, "\t", param.paramType, "\t", param.paramValue});
    ...
    public static void init() {
        ...
        if (GWorld.DEBUG) {
            ...
            final File debugConfigFile = new File(GConfig.getInstance().get("dir.game.data") + "\\config\\GameConfig\\DebugParameters.json");
            JobFactory.addScheduleJob(new Runnable() {
                ...
                public void run() {
                    ...
                    if (newModifiedTime > this.modifiedTime) {
                        ...
                        Out.error(new Object[]{this.val$debugConfigFile.getAbsolutePath(), " has modified, begin reload......."}, );
                        DataUtil.reloadData(debugConfigFile);
                        GlobalConfig.initDebugParameters(config);
                        GuildFortService.getInstance().reload();
                        Out.error(new Object[]{this.val$debugConfigFile.getAbsolutePath(), " reloaded!!!"}, );
                    } else {
        ...
        if (rs != null) {
            for (int k = 0; k < rs.length; k++) {
                mysteriousRewardInfo.put(rs[k], Integer.valueOf(k + 1));
            }
        }
        ...
        List<DungeonMapCO> towers = GameData.findDungeonMaps(p -> (p.type == Const.SCENE_TYPE.DEMON_TOWER.getValue()));


        DemonTowerMapIds = new int[towers.size()];
        for (int i = 0; i < DemonTowerMapIds.length; i++) {
            DemonTowerMapIds[i] = ((DungeonMapCO) towers.get(i)).mapID;
        }
        ...
        String[] exceptTypeStrs = PK_Killed_LostEquip_ExceptType.split(",");
        int j;
        byte b;
        for (rs3 = exceptTypeStrs, j = rs3.length, b = 0; b < j; ) {
            String exceptTypeStr = rs3[b];
            if (StringUtil.isNotEmpty(exceptTypeStr)) {
                PK_EXCEPT_TYPES.add(Integer.valueOf(Integer.parseInt(exceptTypeStr)));
            }

            b++;
        }

        OpenLvCO open = (OpenLvCO) GameData.OpenLvs.get(Integer.valueOf(16));
        if (open != null) {
            Team_Min_Level = open.openLv;
        }
        TeamTargetCO target = (TeamTargetCO) GameData.TeamTargets.get(Integer.valueOf(1010));
        if (target != null) {
