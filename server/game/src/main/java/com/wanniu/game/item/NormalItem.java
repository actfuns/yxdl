...
public class NormalItem {
    ...
    public ItemOuterClass.Item.Builder getView() {
        ItemOuterClass.Item.Builder item = ItemOuterClass.Item.newBuilder();
        item.setId(this.itemDb.id);
        return item;
    }
    ...
    public boolean addGroupNum(int num) {
        synchronized (this.lock) {
            if (this.itemDb.groupCount + num <= this.prop.groupCount) {
    ...
    public void old() {
        this.itemDb.isNew = 0;
        this.itemDb.gotTime = DateUtil.getZeroDate();
    }
    ...
    public int getNum() {
        if (isVirtual()) return getWorth();
        return this.itemDb.groupCount;
    }
    ...
    public boolean isFullGroup() {
        if (this.itemDb.groupCount >= this.prop.groupCount) return true;
        return false;
    }
    ...
    public boolean isBlood() {
        ...
        if (BloodManager.itemFilter.contains(itemCode())) {
            return false;
        }
        return true;
    }
    ...
    public PlayerItemPO cloneItemDB() {
        return (PlayerItemPO) Utils.clone(this.itemDb);
    }

    public ItemOuterClass.Item.Builder toJSON4GridPayload() {
        ItemOuterClass.Item.Builder data = ItemOuterClass.Item.newBuilder();
        data.setId(getId());
        data.setCode(this.itemDb.code);
        data.setItemType((ItemConfig.getInstance().getTypeConfig(this.prop.type)).iD);
        data.setItemSecondType((ItemConfig.getInstance().getIdConfig(this.prop.type)).typeID);
        data.setGroupCount(this.itemDb.groupCount);
        data.setMaxGroupCount(this.prop.groupCount);
        data.setIsNew(this.itemDb.isNew);
        data.setIcon(this.prop.icon);
        data.setQColor(this.prop.qcolor);
        data.setBindType(this.itemDb.isBind);


        data.setStar(0);
        data.setCdTime(String.valueOf(getCD()));
        return data;
    }
    ...
    public final ItemToBtlServerData toJSON4BatterServer(Collection<String> belongPlayerIds, Const.TEAM_DISTRIBUTE_TYPE distributeType, boolean isPlayerDead) {
        ...
        if (!isPlayerDead &&
                !isBinding() &&
                GlobalConfig.getBindByQcolor(this.prop.qcolor) > 0) {
            setBind(1);
        }
    ...
    public final BattleHandler.ItemDrop.Builder toProto4Client() {
        BattleHandler.ItemDrop.Builder data = BattleHandler.ItemDrop.newBuilder();
        data.setId(getId());
        ...
        if (!isBinding() && GlobalConfig.getBindByQcolor(this.prop.qcolor) > 0) {
            setBind(1);
        }
        return data;
    }
    ...
    public final ConsignmentItemsPO toJSON4ConsignmentLine(int salePrice, String playerName, int pro, String playerId, int effectiveTime, int num, int lateMinutes) {
        ...
        if (this.prop.groupCount != 1) {
            ...
            Out.debug(new Object[]{"toJSON4ConsignmentLine generate id  source id:", this.itemDb.id, "targetId:", data.id});
        } else {
        ...
        data.level = getLevel() + getUpLevel() * 1000;
        data.consignmentPrice = salePrice;
        data.consignmentTime = System.currentTimeMillis() + effectiveTime + (lateMinutes * Const.Time.Minute.getValue());
