...
public class PlayerRankHandler
    ...
    public GMResponse execute(JSONArray arr) {
        ...
        list.add(new GmPlayerRankVO("综合", "人物战力榜", (rank == 0L) ? "未上榜" : String.valueOf(rank)));

        rank = RankType.LEVEL.getHandler().getRank(GWorld.__SERVER_ID, id);
        list.add(new GmPlayerRankVO("综合", "人物等级榜", (rank == 0L) ? "未上榜" : String.valueOf(rank)));

        rank = RankType.Mount.getHandler().getRank(GWorld.__SERVER_ID, id);
        list.add(new GmPlayerRankVO("综合", "坐骑榜", (rank == 0L) ? "未上榜" : String.valueOf(rank)));

        rank = RankType.PET.getHandler().getRank(GWorld.__SERVER_ID, id);
        list.add(new GmPlayerRankVO("综合", "宠物榜", (rank == 0L) ? "未上榜" : String.valueOf(rank)));
